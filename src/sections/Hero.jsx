...
const Hero = () => {
    const [selectedShoeURL, setSelectedShoeURL] = useState(bigShoe1)
    ...
    return (
        <section id={'Home'} className={'w-full flex xl:flex-row flex-col justify-center min-h-screen gap-10 max-container'}>
            ...
            <div className={'relative flex-1 flex justify-center items-center xl:min-h-screen max-xl:py-40 bg-primary bg-hero bg-cover bg-center'}>
                <img src={selectedShoeURL} alt={'shoe collection'} width={610} height={500} className={'object-contain relative z-10'}/>
                <div className={'flex sm:gap-6 gap-4 absolute -bottom-[5%] sm:left-[10%] max-sm:px-6'}>
                    {
                        shoes.map((shoe) => (
                            <div key={shoe.thumbnail}>
                                <ShoeCard
                                    shoe={shoe}
                                    changeBigShoeImage={() => setSelectedShoeURL(shoe.bigShoe)}
                                    selectedShoe={selectedShoeURL}
                                />
                            </div>
                        ))
                    }
